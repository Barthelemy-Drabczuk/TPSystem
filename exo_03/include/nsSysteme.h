...
namespace nsSysteme
{
   void        Stat    (const char * file_name, struct stat * buf)
    ...
    int         Open   (const char * pathname, int flags, ::mode_t mode)
                             throw (CExc);
    ::size_t    Read   (int fd, void * buf, std::size_t count)
                             throw (CExc);
...
::ssize_t nsSysteme::Write (int fd, const void *buf, size_t count) throw (CExc) {
	::ssize_t Res;
	if (-1 == (Res = ::write (fd, buf, count)))  throw CExc ("Write()", "n'a pas pu ecrire dans le fichier demande");
	return Res;
} // Write ()

inline 
::size_t nsSysteme::Read (int fd, void * buf, std::size_t count)
    throw (CExc)
{
    ::ssize_t Res;
